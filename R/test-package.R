...
#' Tests should be placed in `tests/autotest`. Use `test_check()` with
#' `R CMD check` and `test_package()` interactively at the console.
#'
#' @section R CMD check:
#' Create `tests/autotest.R` that contains:
#'
#' \preformatted{
#' library(autotest)
#' library(yourpackage)
...
test_package <- function(package, filter = NULL, reporter = default_reporter(), ...) {
  ...
  # If autotest subdir exists, use that
  test_path2 <- file.path(test_path, "autotest")
  if (file.exists(test_path2)) {
    test_path <- test_path2
  } else {
    warning("Placing tests in `inst/tests` is deprecated. ",
      "Please use `tests/autotest` instead", call. = FALSE)
  }
