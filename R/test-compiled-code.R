...
#' for you in `tests/autotest/test-cpp.R` after calling
...
expect_cpp_tests_pass <- function(package) {
  ...
  routine <- get_routine(package, "run_autotest_tests")
...
use_catch <- function(dir = getwd()) {
  ...
  success <- file.copy(
    system.file(package = "autotest", "resources", "test-runner.cpp"),
    test_runner_path,
    overwrite = TRUE
  )
  ...
  success <- file.copy(
    system.file(package = "autotest", "resources", "test-example.cpp"),
  ...
  test_dir <- file.path(dir, "tests", "autotest")
  if (!file.exists(test_dir) && !dir.create(test_dir, recursive = TRUE))
    stop("failed to create 'tests/autotest/' directory '", test_dir, "'", call. = FALSE)

  template_file <- system.file(package = "autotest", "resources", "test-cpp.R")
  ...
  message("> Please ensure you have 'LinkingTo: autotest' in your DESCRIPTION.")
