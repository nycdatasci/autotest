...
ListReporter <- R6::R6Class("ListReporter", inherit = Reporter,
  public = list(
    ...
    get_results = function() {
      autotest_results(self$results$as_list())
    }
  )
)

#' Create a `autotest_results` object from the test results
#' as stored in the ListReporter results field.
#'
#' @param results a list as stored in ListReporter
#' @return its list argument as a `autotest_results` object
#' @seealso ListReporter
autotest_results  <- function(results) {
  stopifnot(is.list(results))
  structure(results, class = "autotest_results")
}
...
all_passed <- function(res) {
  ...
  df <- as.data.frame.autotest_results(res)
  sum(df$failed) == 0 && all(!df$error)
}

#' @export
as.data.frame.autotest_results <- function(x, ...) {
  if (length(x) == 0)
    return(data.frame())
...
#' @export
print.autotest_results <- function(x, ...) {
