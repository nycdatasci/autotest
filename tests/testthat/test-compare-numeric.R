...
test_that("non-numeric types are not compatible", {
  expect_match(compare(1, "a")$message, "returns type")
})

test_that("base lengths must be identical", {
  expect_match(compare(1, c(1, 2))$message, "length")
})
...
test_that("classes must be identical", {
  ...
  expect_match(compare(1L, f1)$message, "factor")
  expect_match(compare(1L, f2)$message, "factor")
})
...
test_that("unnamed arguments to all.equal passed through correctly", {
  expect_equal(415, 416, tolerance = 1)
})
...
test_that("tolerance used for individual comparisons", {
  ...
  expect_true(compare(x1, x2, tolerance = 0.1000001)$equal)
})
